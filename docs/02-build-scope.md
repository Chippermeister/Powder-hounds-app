...
Status: **DRAFT, awaiting approval**. Written 2026-09-25.
...
- **Why PWA first:** one codebase that installs on iOS and Android today. Later, **Capacitor** wraps the *same*
...
*Lesson:* WebGL can only draw images from another domain if that server sends an `Access-Control-Allow-Origin`
